...
casos_con_muni.to_csv("data\processed\datos_casos.csv")
...
def clean_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 1) shapely -> WKT (y renombrar 'geometry' a 'geometry_wkt')
    if "geometry" in out.columns:
        out["geometry_wkt"] = out["geometry"].apply(
            lambda v: v.wkt if isinstance(v, BaseGeometry) else (None if pd.isna(v) else str(v))
        )
    ...
    # 3) NaN -> None y clave a str
    out = out.replace({np.nan: None})
    if KEY in out.columns:
        out[KEY] = out[KEY].astype(str)

    # (opcional) quitar duplicados en el DF por si vienen repetidos
    # out = out.drop_duplicates(subset=[KEY], keep="last")

    return out

# --- 1) DROP + CREATE con esquema explícito ---
schema_sql = f"""
DROP TABLE IF EXISTS "{TABLE}";

CREATE TABLE "{TABLE}" (
    "DIRECCION"              TEXT,
    "ideventocaso"           TEXT NOT NULL,
    "sexo"                   TEXT,
    "edad_diagnostico"       INTEGER,
    "fecha_picadura_estimada" TEXT,
    "fecha_apertura"         TEXT,
    "fis"                    TEXT,
    "lon"                    REAL,
    "lat"                    REAL,
    "WKT"                    TEXT,               -- tu WKT lon/lat si ya lo traes
    "geometry_wkt"           TEXT,               -- WKT proyectado desde 'geometry'
    "distancia_rio"          REAL,
    "distancia_verde_m"      REAL,
    "distancia_grande_m"     REAL,
    "municerca"              TEXT,
    PRIMARY KEY ("ideventocaso")                 -- UNIQUE implícito
);
"""
...
# --- 2) INSERT del DataFrame ---
df_clean = clean_df(casos_con_muni)   # <- tu DF en memoria
engine = create_engine(f"sqlite:///{DB_PATH}", future=True)
df_clean.to_sql(TABLE, con=engine, if_exists="append", index=False, chunksize=1000, method="multi")

print("Reset + carga completa OK.")


# In[ ]:


len(casos_con_muni.drop_duplicates(subset="ideventocaso"))
...
src = Path(r"data\external\infra y mov\Capa de Municercas\NuevaDistribucion2024.shp")  # tu .shp (con .shx/.dbf/.prj al lado)
dst = src.with_name("municercas.geojson")

# si tu DBF tiene acentos raros, prueba encoding="latin1" o "cp1252"
gdf = gpd.read_file(src, engine="pyogrio")  # o quita engine si no tienes pyogrio
# si el shapefile ya tiene CRS conocido, reproyecta a WGS84 para Leaflet:
if gdf.crs is not None:
    gdf = gdf.to_crs(4326)
else:
    # si no tiene .prj, asigna el CRS correcto antes (ej. 22185). NO reproyecta, sólo asigna:
    gdf = gdf.set_crs(4326)  # cambia 4326 por el EPSG real si lo sabes
...
print(gdf.head()[["Nombre"]].head())  # ajusta si el campo se llama distinto


# In[ ]:


casos_con_muni=casos_con_muni.drop_duplicates(subset="ideventocaso")


# In[ ]:


casos_con_muni33=casos_con_muni.copy()
casos_con_muni33["lat + lon"]=casos_con_muni33["lat"].astype(str)+" "+casos_con_muni33["lon"].astype(str)


# In[ ]:


casos_con_muni33[casos_con_muni33.duplicated(subset="ideventocaso", keep=False)]
casos_con_muni33
...
with sqlite3.connect(DB_PATH) as con:
    con.execute(f'CREATE UNIQUE INDEX IF NOT EXISTS uix_{TABLE}_ideventocaso ON "{TABLE}"(ideventocaso);')
